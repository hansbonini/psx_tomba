...
class TombaFunction(object):
    """A representation of a function within Tomba decompilation, including paths and code associated with the function"""
    ...
    def __init__(self, func_root: Path, function_path: Path, args: argparse.Namespace):
        ...
        print(f"Function {self.name} located at {self.src_path} in version {self.version}")
    ...
    def upload_to_decompme(self) -> None:
        """Uploads the decompiled code to decomp.me using the extracted context and Tomba decomp.me presets"""
    ...
    def _infer_src_path(self) -> Optional[Path]:
        inferred_c_name = f"{self.abspath.parent.name}.c"
        inferred_c_files = (
            file
...
def inject_decompiled_function(repo_root: Path, Tomba_func: TombaFunction) -> None:
    ...
    if Tomba_func.include_asm in lines:
        ...
        new_lines[function_index] = Tomba_func.decompile()
        safe_write(Tomba_func.src_path, new_lines)

        print(
            f"{Tomba_func.name} was successfully decompiled, but likely will not compile without adjustments."
        )
        print(
            f"When it successfully compiles, the following command can be used to look for the differences:\n\t{Tomba_func.asm_differ_command}"
        )
    elif [line for line in lines if Tomba_func.name in line]:
        print(
            f"{Tomba_func.name} seems to have already been decompiled in {Tomba_func.src_path.relative_to(repo_root)}"
        )
    else:
        print(
            f"Could not find function {args.function}, are you sure it exists in version {args.version}?"
...
def main(args: argparse.Namespace) -> None:
    ...
    if function_path:
        Tomba_func = TombaFunction(repo_root, function_path, args)
        if Tomba_func.src_path and Tomba_func.decompile():
            inject_decompiled_function(repo_root, Tomba_func)

        if args.upload:
            Tomba_func.upload_to_decompme()
