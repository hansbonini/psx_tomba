...
#define dumpXY4(p)                                                             \
    GPU_printf("(%d,%d)-(%d,%d)-(%d,%d)-(%d,%d)\n", (p)->x0, (p)->y0, (p)->x1, \
               (p)->y1, (p)->x2, (p)->y2, (p)->x3, (p)->y3)

#define dumpUV4(p)                                                             \
    GPU_printf("(%d,%d)-(%d,%d)-(%d,%d)-(%d,%d)\n", (p)->u0, (p)->v0, (p)->u1, \
               (p)->v1, (p)->u2, (p)->v2, (p)->u3, (p)->v3)
...
extern int Krom2Tim(u_char* sjis, u_long* taddr, int dx, int dy, int cdx,
                    int cdy, u_int fg, u_int bg);
